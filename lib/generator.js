...
/**
 * 🆕 按消息相关性加权挑模板（解决"答非所问"）
 * 思路：把消息切成 2~4 字的语义片段，看每个模板命中多少片段
 *  - 命中多的 = 话题对齐 → 加入 top-k 池
 *  - 池子非空时，从 top-k 里随机；池子空时回退纯随机
 * 这样既保证相关性，又保留随机性避免每次都同一句
 */
...
function pickByRelevance(templates, message, vars = {}) {
  ...
  if (chunks.length === 0) return pickRandom(templates);

  // 给每个模板打分：命中一个片段 +1，命中片段越长加权越高
  ...
  if (maxScore === 0) {
    // 没有任何模板命中消息片段 → 走纯随机，但偏向"中等长度"的模板
    // （避免极端短/极端长，那样更容易显得答非所问）
...
export function generateOnePerPersonality(message, opts = {}) {
  ...
  for (const pk of personalityKeys) {
    ...
    const text = humanizeText(rawText, pk, safeIntensity, { message });
    results.push({
      personality: pk,
      label: p.label,
      emoji: p.emoji,
